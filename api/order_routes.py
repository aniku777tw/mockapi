...
async def preview_order(request_body: dict = Body(...)):
    ...
    try:
        ...
        has_normal_case = any(
            key in request_body for key in ["people", "thsr", "roomId", "ratePlanId"]
        )
        has_package_case = any(
            key in request_body
            for key in ["singleCount", "ticketCount", "combinationOptionId"]
        )
        ...
        if has_normal_case and has_package_case:
            error_response = ErrorResponse(
                status=Status(
                    code="CP01005",
                    msg="normal case 欄位與 package case 欄位不能同時存在",
                ),
                data={
                    "normalCaseFields": [
                        key
                        for key in ["people", "thsr", "roomId", "ratePlanId"]
        ...
        if (
            request_body.get("singleCount") is not None
            and request_body.get("ticketCount") is not None
        ):
            error_response = ErrorResponse(
                status=Status(
                    code="CP01005", msg="singleCount 和 ticketCount 其中一者為 null"
                ),
                data={
        ...
        if (
            request_body.get("singleCount") is None
            and request_body.get("ticketCount") is None
        ):
            error_response = ErrorResponse(
                status=Status(
                    code="CP01005", msg="singleCount 和 ticketCount 不能同時為 null"
                ),
                data={
                    "singleCount": request_body.get("singleCount"),
                    "ticketCount": request_body.get("ticketCount"),
        ...
        return CustomResponse(status=Status(code="0", msg="success"), data=mock_data)
    except HTTPException:
        raise
    except Exception as e:
        error_response = ErrorResponse(
            status=Status(code="CP50001", msg="伺服器內部錯誤"), data={"error": str(e)}
        )
